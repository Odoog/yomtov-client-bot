...
class ActionBack(Action):
    def __init__(self):
        super().__init__(lambda scope, user, input_string, bot: user.change_stage(user.stages_history[-2]))


class ActionBackToMainStage(Action):
    def __init__(self):
        super().__init__(lambda scope, user, input_string, bot: user.change_stage(scope.main_stage))
